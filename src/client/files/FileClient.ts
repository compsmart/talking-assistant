...
export function deleteFiles(paths: string[]) { return fetch('/api/workspace/files', { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ paths }) }).then((response) => json<{ value: string[]; publicationId: string; publicationPending: true }>(response)); }
