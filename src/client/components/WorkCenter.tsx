...
interface Props { open: boolean; works: WorkItemSnapshot[]; events: Record<string, ActivityEvent[]>; onClose: () => void; onCancel: (id: string) => void; onApprove: (work: WorkItemSnapshot) => void }
const TERMINAL = new Set(['completed', 'failed', 'cancelled', 'superseded']);

export function WorkCenter({ open, works, events, onClose, onCancel, onApprove }: Props) {
