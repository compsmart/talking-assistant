...
export function AgentsPanel({ workspaceId, onClose, onError }: Props) {
  ...
  const createProfile = () => {
    ...
    const profile: AgentProfile = { id: crypto.randomUUID(), name: 'New agent', description: '', kind: 'custom', enabled: true, stages: ['coder'], capabilities: [], model: 'gemini-2.5-pro', instructions: '', toolIds: [], skillIds: [], contextIds: [], secretGrantIds: [], priority: 0, maxConcurrency: 1, routingRules: [], revision: 1, createdAt: now, updatedAt: now };
    setDraft((current) => current && ({ ...current, profiles: [...current.profiles, profile] })); setSelectedAgentId(profile.id); setTab('agents');
  };
  const cloneProfile = (source: AgentProfile) => { const now = new Date().toISOString(); const copy = { ...structuredClone(source), id: crypto.randomUUID(), name: `${source.name} copy`, kind: 'custom' as const, revision: 1, createdAt: now, updatedAt: now }; setDraft((current) => current && ({ ...current, profiles: [...current.profiles, copy] })); setSelectedAgentId(copy.id); };
...
function ResourcesTab({ snapshot, onChange }: { snapshot: AgentConfigurationSnapshot; onChange: (value: AgentConfigurationSnapshot) => void }) {
  const addSkill = () => { const now = new Date().toISOString(); onChange({ ...snapshot, skills: [...snapshot.skills, { id: crypto.randomUUID(), name: 'New skill', description: '', instructions: '', capabilities: [], requiredToolIds: [], requiredSecretKinds: [], contextIds: [], enabled: true, revision: 1, createdAt: now, updatedAt: now }] }); };
  const addContext = () => { const now = new Date().toISOString(); onChange({ ...snapshot, contexts: [...snapshot.contexts, { id: crypto.randomUUID(), name: 'New context', description: '', scope: 'global', content: '', enabled: true, revision: 1, createdAt: now, updatedAt: now }] }); };
