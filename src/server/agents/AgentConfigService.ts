...
function migrateConfiguration(input: any) {
  ...
  for (const profile of value.profiles || []) {
    const expanded = expand(profile.toolIds); const tools = Array.isArray(expanded) ? expanded : [];
    profile.toolIds = [...new Set([...tools.filter((id) => profile.stages?.includes('media') || !['remove_image_background', 'extract_image_regions'].includes(id)), 'run_node_script'])];
    for (const rule of profile.routingRules || []) if (rule.requiredTools) rule.requiredTools = expand(rule.requiredTools);
  }
  for (const override of value.overrides || []) if (override.toolIds) { const expanded = expand(override.toolIds); override.toolIds = [...new Set([...(Array.isArray(expanded) ? expanded : []), 'run_node_script'])]; }
